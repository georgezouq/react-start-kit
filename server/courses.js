...
let findAll = (req,res,next) => {
    ...
    sql = ` SELECT distinct cou.* FROM course cou 
            INNER JOIN enrollment en on cou.id = en.course_id 
            ${ifStudentId}
            ORDER BY cou.id  DESC LIMIT $1 OFFSET $2`;

    sqlNum = `SELECT distinct count(1) as i FROM course cou INNER JOIN enrollment en on cou.id = en.course_id ${ifStudentId.replace('$3','$1')}`;
